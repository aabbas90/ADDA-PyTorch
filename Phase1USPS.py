...
train_dataset = USPSSAMPLE(root='./data/', num_training_samples=7438, train=True, transform=transforms.ToTensor())

test_dataset = USPSSAMPLE(root='./data/', num_training_samples=1860, train=False, transform=transforms.ToTensor())
...
losses = []# int(math.ceil(float(numIterations) / float(len(train_loader))))
...
torch.save(model, 'preTrainedCNNUSPS')
torch.save(classifier, 'preTrainedClassiferUSPS')